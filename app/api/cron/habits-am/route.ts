...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [{ data: habits }, { data: logs }] = await Promise.all([
      supabase.from('habits').select('id, name').eq('period', 'AM').is('deleted_at', null),
    ...
    if (pending.length === 0) return NextResponse.json({ ok: true, message: 'All AM habits done' });

    // One tap-to-log button per pending habit (max 5 per action row).
    // AM = green (style 3) to match the /habit overview colour scheme.
    ...
    await sendWithButtons(`🌅 **AM habits** — ${pending.length} still pending:`, rows);
    return NextResponse.json({ ok: true });
  } catch (err) {
    // Return 200 so Vercel does NOT retry — error is logged for debugging
    console.error('habits-am cron error:', err);
