...
export function useCadenceData() {
  ...
  const handleAddTodo     = useCallback(async (text: string, priority: 'high' | 'low', category: 'personal' | 'aevro' = 'personal') => { await fetch('/api/todos', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text, priority, category }) }); fetchData(); }, [fetchData]);
  const handleDeleteTodo  = useCallback(async (id: string) => { await fetch(`/api/todos/${id}`, { method: 'DELETE' }); fetchData(); }, [fetchData]);
  const handleEditTodo    = useCallback(async (id: string, text: string, priority: 'high' | 'low') => { await fetch(`/api/todos/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text, priority }) }); fetchData(); }, [fetchData]);
  ...
  return {
    habits, habitLogs, todos, events, notifications,
